      ...
      <SidebarGroup key={section.sectionName}>
        <SidebarMenu>
          {section.tabs.map((item) => (
            <Alert
              key={item.title}
              className={`flex p-2 items-start border-l-4 border-t-0 border-b-0 border-r-0 border-${item.color}-500 bg-${item.color}-100 rounded-sm`}
            >
              ...
              {/* Date Badge */}
              <div className="flex flex-col items-center justify-center w-12 h-12 bg-white border border-gray-300 rounded-md shadow-sm mx-auto">
